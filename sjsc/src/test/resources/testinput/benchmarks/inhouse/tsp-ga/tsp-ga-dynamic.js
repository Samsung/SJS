...
function switchGeneration(){
	...
	chromosomes[currentGeneration] = chromosomes[childGeneration];
	chromosomes[childGeneration] = [];
}
...
function initializeChromosome(generation, chromoIndex){
	...
	generation[chromoIndex]= chromo;

	intHint = chromoIndex;
	intArrArrHint = generation;
}
function initialize(){
	var i=0;
	currentGeneration = 0;
...
function evolve(){
	...
	for(i=0 ; i<population ; i++){
		...
		chromoC = [];		
		intArrHint = chromoC;
		chromosomes[childGeneration][i] = chromoC;
...
function run(){
	...
	customerNumber = getIntArg(1);
	population = getIntArg(2);
	generationGoal = getIntArg(3);
	//baseX = getIntArg(2);
	//baseY = getIntArg(3);
	baseX = rand(0,1000);
	baseY = rand(0,1000);
	...
	for(i=0;i<customerNumber;i++){
		for(j=i+1;j<customerNumber;j++){
			...
			distance[i][j] = (sqrt(t1 * t1 + t2 * t2) * 1000) | 0;
			distance[j][i] = distance[i][j];
		}
		t1 = customerX[i] - baseX;
		t2 = customerY[i] - baseY;
		baseDistance[i] = (sqrt(t1 * t1 + t2 * t2) * 1000) | 0
	}
