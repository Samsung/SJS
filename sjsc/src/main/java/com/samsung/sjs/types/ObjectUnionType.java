...
public class ObjectUnionType implements Type, Iterable<ObjectType> {
    ...
    public String generateTag(TypeTagSerializer tts) {
        int len = objTypes.size();
        assert len > 0;
        if (len > 1) {
            System.err.println("WARNING: Attempting to serialize tag for object union (will fail for non-equivalent union members");
        }

        ObjectType base = objTypes.get(0);

        for (ObjectType t : objTypes) {
            if (t == base) {
                continue;
            }
            if (!Types.isEqual(base,t)) {
                throw new IllegalArgumentException("Attempted to generate tag for union of non-identical types!  Union in question: "+this);
            }
        }

        return base.generateTag(tts);
