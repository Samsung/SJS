...
public final class IRCBackend extends CBackend {
    ...
    public static void exportPropertyOffsets(CompilationUnit ccode, IRFieldCollector.FieldMapping m) {
        ...
        ccode.addStatement(new com.samsung.sjs.backend.asts.c.ExpressionStatement(new InlineCCode("const int ___propname_count = "+names_by_id.length+";")));
    }
    ...
    public void generateTypeViolationTransition(com.samsung.sjs.backend.asts.c.CompoundStatement cs, String jump_target, Var tmp) {
        ...
        if (tmp != null && tmp.getIdentifier().startsWith("__tmp")) {
            bs.addStatement(new com.samsung.sjs.backend.asts.c.ExpressionStatement(
                        new Assignment(new Variable("__slow_"+tmp.getIdentifier()), "=", new Variable(tmp.getIdentifier()))));
        }
    ...
    public com.samsung.sjs.backend.asts.c.Statement visitVarDecl(VarDecl node) {
        ...
        if (node.getVar().asVar().getIdentifier().startsWith("__tmp")) {
            ...
            if (options.interopEnabled() && node.getInitialValue().isCall()) {
                ...
                generateTypeViolationTransition(cs, "__dirty_call_transition"+fcall.callsiteNumber(), node.getVar().asVar());
                return cs;
            }
    ...
    @Override
    public com.samsung.sjs.backend.asts.c.Statement visitExpressionStatement(com.samsung.sjs.backend.asts.ir.ExpressionStatement node) {
        ...
        if (result instanceof com.samsung.sjs.backend.asts.c.Expression) {
            ...
            com.samsung.sjs.backend.asts.c.ExpressionStatement ret =
                new com.samsung.sjs.backend.asts.c.ExpressionStatement(result.asExpression());
            if (options.interopEnabled()) {
                com.samsung.sjs.backend.asts.c.CompoundStatement cs = new com.samsung.sjs.backend.asts.c.CompoundStatement();
                cs.addStatement(ret);
        ...
        switch (node.getOp()) {
            ...
            case "==":
                if (node.getLeft().getType() instanceof ObjectType || node.getRight().getType() instanceof ObjectType) {
                    assert(node.getLeft().getType() instanceof ObjectType && node.getRight().getType() instanceof ObjectType);
    ...
    public CNode visitAllocNewObject(AllocNewObject node) {
        ...
        f.addActualArgument(new InlineCCode(new CastExpression(c_ctor_type, vtmp.inType(node.getConstructor().getType())).toSource(0)+"->prototype.obj")); // get proto from the closure
    ...
    public CNode visitMethodCall(MethodCall node) {
        ...
        com.samsung.sjs.backend.asts.c.FunctionCall f =
            new ClosureCall(method_lookup, c_slot_type);
    ...
    public CNode visitUntyped(UntypedAccess node) {
        ...
        if (node.untypedVariable().isVar()) {
            switch (node.getType().rep()) {
                ...
                case CODE:
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("Trying to emit untyped access to ["+node.toSource(0)+"] of rep sort "+node.getType().rep());
            }
        } else {
        ...
        if (node.untypedVariable().isVar()) {
            f.addActualArgument(new InlineCCode("&"+node.untypedVariable().accept(this).asExpression().toSource(0)));
        } else {
    ...
    public class SlowPathGenerator extends CBackend {
        ...
        public com.samsung.sjs.backend.asts.c.Statement visitVarDecl(VarDecl node) {
            ...
            com.samsung.sjs.backend.asts.c.ExpressionStatement ret = new com.samsung.sjs.backend.asts.c.ExpressionStatement(wr);
            // Emit a label after the call as a jump target from the type-correct path
        ...
        public com.samsung.sjs.backend.asts.c.Expression visitArrayIndex(ArrayIndex node) {
            if (node.getArray().getType() instanceof EnvironmentType) {
                ...
                com.samsung.sjs.backend.asts.c.FunctionCall f = new com.samsung.sjs.backend.asts.c.FunctionCall(lval_capture ? "OP_GETINDEXUNBOXED" : "OP_GETINDEXSTARUNBOXED");
                Var envname = (Var)node.getArray();
                com.samsung.sjs.backend.asts.ir.IntLiteral offset = (com.samsung.sjs.backend.asts.ir.IntLiteral)node.getOffset();
                int ioffset = (int)offset.getValue();
                f.addActualArgument(node.getArray().accept(this).asExpression());
            ...
            com.samsung.sjs.backend.asts.c.FunctionCall f = new com.samsung.sjs.backend.asts.c.FunctionCall("OP_GETPROP");
            f.addActualArgument(node.getArray().accept(this).asExpression());
            f.addActualArgument(node.getOffset().accept(this).asExpression());
            return f;
        }
        ...
        public com.samsung.sjs.backend.asts.c.Expression visitFieldAssignment(FieldAssignment node) {
            ...
            com.samsung.sjs.backend.asts.c.FunctionCall f = new com.samsung.sjs.backend.asts.c.FunctionCall("OP_SETPROP");
            f.addActualArgument(node.getObject().accept(this).asExpression().asValue(node.getObject().getType()));
            // align the string constant
            f.addActualArgument(new Str(node.getField()).accept(this).asExpression().asValue(Types.mkString()));
        ...
        public CNode visitReturn(Return node) {
            ...
            // stack
            return IRCBackend.this.visitSlowReturn(node, node.hasResult() ? node.getResult().accept(this).asExpression() : null);
        }
        @Override
        public CNode visitForInLoop(ForInLoop node) {
        ...
        public com.samsung.sjs.backend.asts.c.Expression visitVarAssignment(VarAssignment node) {
            if (node.getAssignedVar() instanceof ArrayIndex) {
                ...
                if (ai.getArray().getType() instanceof EnvironmentType) {
                    com.samsung.sjs.backend.asts.c.FunctionCall f = new com.samsung.sjs.backend.asts.c.FunctionCall("OP_SETINDEXSTARUNBOXED");
                    Var envname = (Var)ai.getArray();
                    com.samsung.sjs.backend.asts.ir.IntLiteral offset = (com.samsung.sjs.backend.asts.ir.IntLiteral)ai.getOffset();
                    int ioffset = (int)offset.getValue();
                ...
                Assignment asgn;
                asgn = new Assignment(node.getAssignedVar().accept(this).asExpression(),
                                      node.getOperator(),
            ...
            switch (node.getOp()) {
                ...
                case "--":
                    if (node.getExpression().isVar() ||
                        (node.getExpression() instanceof ArrayIndex && ((ArrayIndex)node.getExpression()).getArray().getType() instanceof EnvironmentType)
                            ) {
                        f = new com.samsung.sjs.backend.asts.c.FunctionCall("UNARY_"+(node.isPostfix() ? "POST" : "PRE")+(node.getOp().equals("++") ? "_INC" : "_DEC"));
                        primret = true;
                        break;
                    } else if (node.getExpression() instanceof FieldRead) {
                        // TODO: predicted field reads
                        f = new com.samsung.sjs.backend.asts.c.FunctionCall("OP_field_"+(node.getOp().equals("++") ? "inc" : "dec"));
                        FieldRead fr = (FieldRead)node.getExpression();
        ...
        public CNode visitAllocNewObject(AllocNewObject node) {
            ...
            //f.addActualArgument(new com.samsung.sjs.backend.asts.c.NullLiteral()); // TODO: get proto from the closure
            com.samsung.sjs.backend.asts.c.FunctionCall proto_lookup = new com.samsung.sjs.backend.asts.c.FunctionCall("OP_GETPROP");
            ...
            com.samsung.sjs.backend.asts.c.FunctionCall ctor_call = new com.samsung.sjs.backend.asts.c.FunctionCall("TYPEDCALL");
            // construct_object returns object_t*
            ctor_call.addActualArgument(new ValueCoercion(ty, f, options.encodeVals()));
        ...
        public CNode visitMethodCall(MethodCall node) {
            ...
            com.samsung.sjs.backend.asts.c.FunctionCall f = new com.samsung.sjs.backend.asts.c.FunctionCall("TYPEDCALL");
        ...
        public CNode visitFunctionCall(com.samsung.sjs.backend.asts.ir.FunctionCall node) {
            ...
            if (e instanceof IntrinsicName || e.toSource(0).equals("assert")) { // IRCBackend::visitFunctionCall has a litany of special cases...
                ...
                assert(!functions.isEmpty());
                com.samsung.sjs.backend.asts.ir.Expression env = null;
                if (node.getDirectCallTarget().getEnvironmentName().equals(IRCBackend.this.functions.peek().getEnvironmentName())) {
                    env = new Var((node.getDirectCallTarget().getEnvironmentName()));
                } else {
                    ...
                    env = new ArrayIndex(envname,
                                         new com.samsung.sjs.backend.asts.ir.IntLiteral(
                                             IRCBackend.this.functions.peek().getEnvLayout().getOffset(
                                                node.getDirectCallTarget().getEnvironmentName())));
                }
                lval_capture = true;
                f.addActualArgument(env.accept(this).asExpression());
