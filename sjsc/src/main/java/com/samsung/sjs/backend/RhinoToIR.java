...
public class RhinoToIR extends ExternalRhinoVisitor {
    ...
    */
    ...
    public static boolean looksLikeModuleImport(org.mozilla.javascript.ast.FunctionCall node,
                                                Map<AstNode,Type> types,
                                                com.samsung.sjs.backend.asts.ir.Scope currentScope) {

        if (node.getTarget() instanceof Name && ((Name)node.getTarget()).getIdentifier().equals("require")) {
            if (currentScope == null || currentScope.lookupType(new Var("require")) == null) {
                return true;
            }
        }
        return false;
    ...
    protected void visitFunctionCall(org.mozilla.javascript.ast.FunctionCall node) {
        ...
        if (isMethodCall) {
            ...
            // TODO: Transform requires
            if (looksLikeModuleImport(node, types, currentScope)) {
                //Type t = types.get(node);
                AstNode arg = node.getArguments().get(0);
                visit(arg);
                lastChildResult = new Require(((Str)lastChildResult.asExpression()).getValue());
                lastChildResult.setType(t);
                return;
            }
            visit(node.getTarget());
