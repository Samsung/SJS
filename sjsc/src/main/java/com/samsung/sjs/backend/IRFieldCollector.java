...
public final class IRFieldCollector extends VoidIRVisitor {
    ...
    public IRFieldCollector(JSEnvironment env, ModuleSystem modsys) {
        ...
        for (Map.Entry<String,Type> kv : modsys.moduleTypes().entrySet()) {
            // the key here is the basename of a module import
            // TODO: Currently this corresponds to untyped code, so the choice of offset is
            // irrelevant.  Once we start importing typed code, we need to coordinate between typed
            // compilations (most likely by compiling dependent modules within this same compiler
            // run, sharing the field layouts.
            Type t = kv.getValue();
            collectFields(t);
        }
