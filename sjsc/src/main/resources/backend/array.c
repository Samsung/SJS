...
UNBOX_NATIVE_METHOD_NONVOID(array_pop, array_pop);
UNBOX_NATIVE_METHOD_NONVOID(array_push, array_push);
UNBOX_NATIVE_METHOD_NONVOID(array_shift, array_shift);
UNBOX_NATIVE_METHOD(array_unshift, array_unshift);

object_t* mk_array_obj(type_tag_t *tag, unsigned int store_size, unsigned long long nelems, value_t* backing) {
...
object_t* inner_Array(type_tag_t *tag, int length) {
    ...
    return mk_array_obj(tag, length, length, backing);
}
value_t clos_Array(env_t env, value_t self, value_t length) {
    // TODO: Remove this hack once we actually convert Array to a construtor!
    return object_as_val(inner_Array(self.ptr != NULL ? val_as_object(self)->type : NULL, val_as_int(length)));
}
NEW_BOX_NATIVE_CTOR(Array, clos_Array);
...
object_t* array___lit(type_tag_t *tag, int length, ...) {
    ...
    return mk_array_obj(tag, length+1, length, backing);
}
