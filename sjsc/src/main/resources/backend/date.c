...
BOX_NATIVE_METHOD(_date_getTime, _date_getTime);
...
BOX_NATIVE_METHOD(_date_getTimezoneOffset, _date_getTimezoneOffset);

value_t _date_valueOf(env_t env, value_t self) {
    jsdate_t* dt = (jsdate_t*)self.obj->fields[DATE_N_METHODS].ptr;
    return int_as_val(dt->tv.tv_sec * 1000 + dt->tv.tv_usec / 1000);
}
BOX_NATIVE_METHOD(_date_valueOf, _date_valueOf);
...
BOX_NATIVE_METHOD(_date_toDateString, _date_toDateString);
...
BOX_NATIVE_CTOR(_ctor_Date, _ctor_Date);
