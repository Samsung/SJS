...
void* closure_alloc_prim(void* env, void* code) {
    void** mem = (void**)aligned_alloc(2*sizeof(void*));
    mem[0] = env;
    mem[1] = code;
    last_alloc_attempt = mem;
    assert ((((uint64_t)mem) & 0x7) == 0ULL);
    return mem;
}
void* ctor_alloc_prim(void* env, void* code, void* proto) {
    void** mem = (void**)aligned_alloc(3*sizeof(void*));
    mem[0] = env;
    mem[1] = code;
    mem[2] = proto;
    return mem;
...
wchar_t* string_fromCharCode_method(env_t e, wchar_t* o, int32_t code) {
    return __str__fromCharCode(code);
}
struct { env_t env; wchar_t* (*func)(env_t, wchar_t*, int32_t); } string_fromCharCode_clos = { NULL, string_fromCharCode_method };
value_t string_fromCharCode_box = (value_t)(void*)&string_fromCharCode_clos;
