...
inner_map_t* __fresh_propbag(int init_props) {
    ...
    inner_map_t* m = (inner_map_t*)MEM_ALLOC(sizeof(inner_map_t));
    m->nbuckets = nbuckets;
    m->mask = mask;
    m->buckets = buckets;
    return m;
...
map_t* __alloc_map(int32_t init_props) {
    map_t *omap = (map_t*)blank_obj(0);
    omap->__propbag = __fresh_propbag(init_props);
    return omap;
}

bool __map_contains(map_t* om, wchar_t* prop) {
    inner_map_t *imap = om->__propbag;
    uint32_t hash = XXH32(prop, wcslen(prop), 0);
    uint32_t bucket = hash & imap->mask;
    node_t* cell = imap->buckets[bucket];
    while (cell != NULL) {
...
value_t __map_access(map_t* om, wchar_t* prop) {
    inner_map_t *imap = om->__propbag;
    uint32_t hash = XXH32(prop, wcslen(prop), 0);
    uint32_t bucket = hash & imap->mask;
    node_t* cell = imap->buckets[bucket];
...
value_t __map_store(map_t* om, wchar_t* prop, value_t v) {
    inner_map_t *m = om->__propbag;
    uint32_t hash = XXH32(prop, wcslen(prop), 0);
...
map_t* __alloc_map_literal(type_tag_t *tag, int n, ...) {
...
// TODO: shrink
bool __map_delete(map_t* om, wchar_t* prop) {
    inner_map_t *m = om->__propbag;
    uint32_t hash = XXH32(prop, wcslen(prop), 0);
    uint32_t bucket = hash & m->mask;
