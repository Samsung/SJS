...
BOX_NATIVE_METHOD(console_log, console_log);

void console_assert(env_t env, value_t objthis, value_t b); // declare only, defined at EOF
BOX_NATIVE_METHOD(console_assert, console_assert);

void console_error(env_t env, value_t objthis, value_t str) {
    fwprintf(stderr, L"%ls\n", val_as_string(str));
}
BOX_NATIVE_METHOD(console_error, console_error);
BOX_NATIVE_METHOD(console_warn, console_error);
...
struct { env_t env; value_t (*func)(env_t, value_t); } object_toString_clos = { NULL, object_toString };
value_t object_toString_box = (value_t)(void*)&object_toString_clos;
