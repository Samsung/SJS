...
class ArrayType:
    ...
    def __str__(self):
        return "array<%s>" % str(self.elemty)
    def toJSON(self):
    ...
    def randomInstance(self):
        ...
        for i in range(0,size):
            out.append(self.elemty.randomInstance())
        return "["+", ".join(out)+"]"
...
''' % (sig , body , cpp_mname , cpp_mname);
    def unwrap(self, val_str):
        ...
        # levels of wrapping
        # TODO: This method has never actually been tested, isn't run with the current IDL input,
        # and now refers to a deleted runtime primitive (INVOKE_CLOSURE)
